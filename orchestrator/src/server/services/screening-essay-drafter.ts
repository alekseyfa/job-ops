...
Output only the answer text, no preamble or explanation.`;
  ...
  try {
    const resp = await llm.callJson<{ answer: string }>({
      model,
    ...
    if (!resp.success) {
      // Route failures through the standard LLM error contract (same as
      // scorer.ts): config-class pauses the flow, transient is a per-call blip.
      const kind = classifyLlmError(resp.error);
      if (kind === "config") {
        throw new LlmNotConfiguredError(
          `AI essay drafting failed: ${resp.error}. Check your LLM configuration in Settings.`,
        );
      }
      throw new LlmTransientError(
        `AI temporarily unavailable: ${resp.error}`,
        resp.error,
      );
    }
