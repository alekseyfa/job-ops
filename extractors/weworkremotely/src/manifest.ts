...
import { runWeWorkRemotely } from "./run";
...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `WeWorkRemotely: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `WeWorkRemotely: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
