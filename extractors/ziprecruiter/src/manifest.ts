...
import { runZipRecruiter } from "./run";
...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `ZipRecruiter: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `ZipRecruiter: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runZipRecruiter({
