...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `HH.ru: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `HH.ru: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}

export const manifest: ExtractorManifest = {
  id: "hhru",
  displayName: "HeadHunter (hh.ru)",
  providesSources: ["hhru"],
